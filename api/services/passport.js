 ...
 * Passport Service
 * 
 * A painless Passport.js service for your Sails app that is guaranteed to Rock
 * Your Socks™. It takes all the hassle out of setting up Passport.js by
 ...
 * 
 * 1. Given a provider and an identifier, find a matching Passport. 2. From
 * here, the logic branches into two paths. - A user is not currently logged in:
 * 1. If a Passport wasn't found, create a new user as well as a new Passport
 * that will be assigned to the user. 2. If a Passport was found, get the user
 * associated with the passport. - A user is currently logged in: 1. If a
 * Passport wasn't found, create a new Passport and associate it with the
 * already logged in user (ie. "Connect") 2. If a Passport was found, nothing
 * needs to happen.
 * 
	...
	Passport.findOne({
		...
		if (!req.user) {
			...
			// Action: Get the user associated with the passport.
				...
				// If the tokens have changed since the last session, update
				// them
...
passport.callback = function(req, res, next) {
	var provider = req.param('provider', 'local'), action = req.param('action');

	// Passport.js wasn't really built for local user registration, but it's
	// nice
	...
	if (provider === 'local' && action !== undefined) {
		...
		if (action === 'disconnect' && req.user) {
			...
			// The provider will redirect the user to this URL after approval.
			// Finish
			// the authentication process by attempting to obtain an access
			// token.
			// If
 ...
 * 
 * For example, we could add this to our config to use the GitHub strategy with
 * permission to access a users email address (even if it's marked as private)
 * as well as permission to add and update a user's Gists:
 * 
 * github: { name: 'GitHub', protocol: 'oauth2', strategy:
 * require('passport-github').Strategy scope: [ 'user', 'gist' ] options: {
...
passport.loadStrategies = function() {
	...
	Object.keys(strategies).forEach(function(key) {
		...
		if (key === 'local') {
			// Since we need to allow users to login using both usernames as
			// well as
			...
			// Let users override the username and passwordField from the
			// options
			...
			var protocol = strategies[key].protocol, callback = strategies[key].callback;
			...
			// Merge the default options with any options defined in the config.
			// All
			// defaults can be overriden, but I don't see a reason why you'd
			// want to
			// do that.
			_.extend(options, strategies[key].options);
...
passport.disconnect = function(req, res, next) {
	var user = req.user, provider = req.param('provider', 'local'), query = {};
